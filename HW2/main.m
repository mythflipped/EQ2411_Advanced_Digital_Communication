...
close all
  ...
  % -> specify the existing check node degrees in a vector   % change Distr_2A to change the check node degress
  Distr_2A.degree   =  [ 6 ];
  
  % -> specify the fractions of the degrees in another vector and ensure
  % that everything sums up to 1  % Distr_2A.farction must sum up to 1, e.g. [0.2 0.8]
  Distr_2A.fraction =  [ 1  ];  
  ...
  for ii_sim = 1:N_simulations
    ...
    for ii_e = 1:length(epsilon)
      ...
      c = ones(1,n);      %%dd=rand(1,n)<epsilon(ii_e);
